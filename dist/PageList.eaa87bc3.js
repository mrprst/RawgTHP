  ...
  var preparePage = function preparePage() {
    ...
    var displayResults = function displayResults(articles) {
      ...
      var resultsContent = articles.map(function (article) {
        return "<article class=\"cardGame\">\n          <div id=\"".concat(article.id, "\"><img src=\"").concat(article.background_image, "\" style=\"width: 200px\"></div>\n          <h1>").concat(article.name, "</h1>\n          <div class=\"game-platforms\"></div>\n          <a href=\"#pagedetail/").concat(article.id, "\">").concat(article.id, "</a>\n        </article>");
      });
      var resultsContainer = document.querySelector(".page-list .articles");
      resultsContainer.innerHTML += resultsContent.join("\n");
      articles.forEach(function (e) {
        var gameId = document.getElementById(e.id);
        showDetails(e, gameId);
        GamePlatforms(e, gameId.nextElementSibling.nextElementSibling);
      });
    };

    var fetchList = function fetchList(url, argument, pagenumber) {
      var finalURL = argument ? "".concat(url, "&search=").concat(argument, "&page=").concat(pagenumber) : "".concat(url, "&page=").concat(pagenumber);
...
var Platforms = function Platforms() {
  var platformDropdown = document.querySelector("#platform");
  fetch('https://api.rawg.io/api/platforms/lists/parents?key=774be2a6817d4369bef88bd479e5ab73').then(function (response) {
    return response.json();
  }).then(function (response) {
    response.results.forEach(function (element) {
      platformDropdown.innerHTML += "\n      <option value=\"".concat(element.name, "\">").concat(element.name, "</option>\n      ");
    });
  });
};
...
function GamePlatforms(game, container) {
  var gamePlatformsContent = game.parent_platforms.map(function (e) {
    return "<i class=\"fab fa-".concat(e.platform.slug, "\"></i>");
  });
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/PageList.js"], null)
